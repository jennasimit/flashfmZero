...
```{r,eval=FALSE} 
library(flashfmZero)
latent_gwas <- latentGWAS(obsGWAS = sel_obsgwas,covY = sel_covY,L = FAloading, beta_colname="beta",se_colname="SE",
                       snpID_colname="rsID",EAF = sel_obsgwas[[1]]$EAF)
```
...
(a) **rsID**: (variant ID names - if using the JAM functions, variant names must not contain ":" and must start with a non-numeric character, see [Notes](#notes) for details). All rsIDs must be unique, i.e. no duplicate names. 
...
#' set NCORES=M (number of traits) or NCORES=1 if running on hpc (do not set an arbitrary number of cores), 
