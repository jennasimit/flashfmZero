...
file_FMresults <- sprintf("output/%d.Rdata",array_index)
...
1. Set argument NCORES=length(fm_traits_latent) in FLASHFMZEROwithJAMd for parallelisation, and set NCORES=length(fm_traits_obs) in multiJAMd. Do not use an arbitrary number for NCORES.
OR
