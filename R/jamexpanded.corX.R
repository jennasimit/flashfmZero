# an updated new function with the same name is in the updated-flashfm-functions.R
...
#     return(df)
# }
#updated from new code: update-flashfm-tagexpand.mod
tagexpand.mod <- function (snpmod, taglist) {
  if (snpmod == "1") {
    df <- data.frame(str = snpmod, snps = snpmod, size = 0, 
                     tag = FALSE, stringsAsFactors = FALSE)
    ...
    tsnps <- vector("list", ns)
    for (i in 1:ns) {
      csnp <- paste0("\\b",snps[i],"\\b")
      tsnps[[i]] <- unique(unlist(taglist[grep(csnp,taglist)]))
    }
    emods <- expand.grid(tsnps, stringsAsFactors = FALSE)
    out <- apply(emods, 1, function(x) {
      paste0(x, collapse = "%")
...
#' @export
#' @author Jenn Asimit
JAMexpandedCor.multi <- function(beta1, corX, raf, ybar, Vy, N, maxcv=10,r2 = 0.99, save.path) 
{
...
#    for (i in 1:M) snps <- union(snps, names(beta1[[i]]))
    ...
    for (j in 1:M) {   
            ...
            jam.results <- JAM(marginal.betas = BETA,                              
