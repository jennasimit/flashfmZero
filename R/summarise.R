...
#' @param snpGroups  list of two sets of snp groups; output from makeSNPgroups2; setting snpGroups=NULL will return only SNP-level results
#' @param minPP a single value such that output for groups consists of group models where PP > minPP for at least one set of fine-mapping results; default is 0.01 
...
#' @author Jenn Asimit
#' @export
PPsummarise <- function (fm.multi, snpGroups, minPP=0.01) 
# for one TOdds setting in fm.multi, snpGroups is a list of 2 snpgroups
{
	...
	if(!is.null(snpGroups)) {
	  ...
	  colnames(outMPP[[i]]) <- fm.multi$sharing
	   ...
	   names(outMPPg) <- names(outPPg) <- tnames
	   names(outMPP) <- names(outPP)  <- tnames
	   
	   }
	   	   
	    for(i in 1:M) {
	    ind <- order(outMPP[[i]][,2],decreasing=TRUE)
	    outMPP[[i]] <- outMPP[[i]][ind,]
	   }
	    for(i in 1:M) {
	    ind <- order(outPP[[i]][,2],decreasing=TRUE)
	    outPP[[i]] <- outPP[[i]][ind,]
	   }
	   
	   OUT <- list(MPP=outMPP, MPPg=outMPPg, PP=outPP, PPg=outPPg)
	    }
	  ...
	  if(is.null(snpGroups)){
	   ...
	   outPP <- fm.multi$PP 
	   for(i in 1:M) outMPP[[i]] <- t(MPP[(2*i-1):(2*i),])  
