...
#' @param vars.to.include Check NULL values.
#' @return NA
...
TracePlots <- function(
  ...
  results@mcmc.output <- cbind("ModelDimension"=apply(results@mcmc.output, MARGIN=1, function(x) sum(x!=0) ), results@mcmc.output)
