...
#' @param credible.percentile.threshold Percentile to determine credible set for (default 0.95 for a 95 percent Credible Set).
...
CredibleSet <- function(
  ...
  ### Error checks
	...
	if (results@enumerate.up.to.dim==0) {
	  ...
	  models.table <- sort(table(all.models),decreasing=T)/nrow(results@mcmc.output) # Tabulates to unique models
	  ...
	  for (ld.block in 1:results@n.covariate.blocks.for.jam) {
	    ...
	    enumerated.model.probs <- sort(enumerated.model.probs, decreasing=T)
