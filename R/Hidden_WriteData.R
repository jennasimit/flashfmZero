...
#' @param model.tau model.tau
#' @param z z
#' @param mrloss.marginal.causal.effects mrloss.marginal.causal.effects
#' @param trait.variance trait.variance
#' @param mrloss.marginal.causal.effect.ses mrloss.marginal.causal.effect.ses
#' 
#' @return NA
...
.WriteData <- function(
  ...
  if (likelihood %in% c("JAM", "JAM_MCMC")) { # Write summary data
    ...
    if (likelihood == "LinearConj") {
      ...
      data <- apply(data,MARGIN=2,function(x) x-mean(x))
    }
